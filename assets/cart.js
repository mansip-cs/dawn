...
//     const giftThreshold =  window.giftThreshold;
//   const giftVariantId = window.giftVariantId;
  ...
  //      
  //       if (cartTotal >= giftThreshold && !hasGift) {
...
//           })
//         }).then(() => {
//           publish(PUB_SUB_EVENTS.cartUpdate, { source: 'free-gift-remove' });
//         });
...
//       document.querySelectorAll('.select-gift-btn').forEach(btn => {
//         btn.disabled = true;
//         btn.innerText = "Added";
//       });
...
function updateButtons(activeGift) {

  document.querySelectorAll('.select-gift-btn').forEach(btn => {

    const id = btn.dataset.variant;

    if (activeGift && activeGift.variant_id == id) {
      btn.disabled = true;
      btn.innerText = "Added";
    } else {
      btn.disabled = false;
      btn.innerText = "Add Gift";
    }
...
document.addEventListener('click', function(e) {
  ...
  const newId = btn.dataset.variant;
  ...
  fetch('/cart.js')
    ...
    .then(cart => {
      ...
      .then(() => {
        publish(PUB_SUB_EVENTS.cartUpdate, { source: 'free-gift-switch' });
      });
...
subscribe(PUB_SUB_EVENTS.cartUpdate, freeGiftHandler);
document.addEventListener('DOMContentLoaded', freeGiftHandler);